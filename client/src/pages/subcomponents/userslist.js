...
import {unfollow} from '../../actions/admin/getuseractions';
 ...
 useEffect(() => {
	...
	return(
	<>
		<div style={{backgroundColor:"#394264",borderRadius: "5px",marginTop:"20px"}}>
		<div style={{borderTopRightRadius:"5px",borderTopLeftRadius:"5px",padding:"12px 0px" ,textAlign:"center",fontSize:"30px",background:"#11a8ab"}}>Following</div>
		{
			userlist && userlist.length > 0 ? userlist.map((u,index)=>{ return (
			<div style={{margin:"20px"}} key={index}>
			<p style={{display:"inline-block"}}><img src={u.pofilePicture ? u.pofilePicture:dummyimg } style={{width:"50px",heigh:"50px",borderRadius:"50%"}}/></p>
			...
			<p style={{color:"white",width:"50%"}}>{u.firstName} </p>
			</Link>
			{ userred._id == props.displayuser ? 
			(<p style={{background :"#009DDC",width:"70px",borderRadius:"5px",height:"20px",textAlign:"center"}} onClick={()=>{setUnfollowid(u._id)}}>
			Unfollow
			</p> 
